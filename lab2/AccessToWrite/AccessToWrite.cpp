...
void AddRecord(HANDLE hFile, const char* string) {
    Header header;
...
void ShowRecord(HANDLE hFile, int recordNumber) {
    ...
    if (recordNumber <= header.recordCount && 0 < recordNumber) {
        ...
        std::cout << "Wrong record number. There are " << header.recordCount << " records in total.";
    }
}

void ChangeRecord(HANDLE hFile, int recordNumber, const char* string) {
    Header header;
    SetFilePointer(hFile, 0, NULL, FILE_BEGIN);
    ReadFile(hFile, &header, sizeof(Header), NULL, NULL);
...
int _tmain(int argc, TCHAR** argv) {
    ...
    if (hFile == INVALID_HANDLE_VALUE) {
        ...
        if (hFile != INVALID_HANDLE_VALUE) {
            ...
            std::cout << "The created file is open..\n";
        }
        else {
    ...
    while (true) {
        ...
        switch (choice) {
        case 1:
            std::cout << "Add some text for a new record (maximum " << TEXT_SIZE << " characters):" << std::endl
                << std::endl;
            std::cin.ignore();
            ...
            break;
        case 2:
            std::cout << "Enter the number of the existing record: ";
            std::cin >> number;
        ...
        case 3:
            ...
            std::cout << "Add some text to change (maximum " << TEXT_SIZE << " characters):" << std::endl
                << std::endl;
            std::cin.ignore();
            std::cin.getline(text, TEXT_SIZE);
